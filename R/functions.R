    ...
    if (!is.null(Evaled[[name]]$aic)) {
      ...
      if (cur_aic < min_aic) {
        ...
        if ("Pt-AR1" %in% row.names(coefz) & "Pt-MA1" %in% row.names(coefz))
        {
          ...
          if(is.nan(PtAR1p)|is.nan(PtMA1p))
          { }
          else if(PtAR1p < 0.05 & PtAR1f > 0 & PtAR1f < 1 & PtMA1p < 0.05 & (PtMA1f * PtMA0f > 0))
          {
            min_aic <- cur_aic
            min_ele <- name
          }else
        ...
        } else if ("Pt-AR1" %in% row.names(coefz))
        {
          ...
          if(is.nan(PtAR1p))
          { }
          else if(PtAR1p < 0.05 & PtAR1f > 0 & PtAR1f < 1)
          {
            min_aic <- cur_aic
            min_ele <- name
          }else
        ...
        else if("Pt-MA1" %in% row.names(coefz))
        {
          ...
          if(is.nan(PtMA1p))
          { }
          else if (PtMA1p < 0.05 & (PtMA1f * PtMA0f > 0))
          {
            min_aic <- cur_aic
            min_ele <- name
          }else
        ...
        else if("St-MA1" %in% row.names(coefz))
        {
          ...
          if(is.nan(StMA1p))
          { }
          else if (StMA1p < 0.05 & (StMA0f * StMA1f > 0))
          {
            min_aic <- cur_aic
            min_ele <- name
          }else
...
Optarimax <- function(y,order,xtransf,transfer,inintra)
{
  ...
  curmod <- arimax(y, order = order, xtransf = xtransf, transfer= transfer, method = "ML",init= inincoef)
  while(1)
  {
    cur_init <- c(rep(NA, pqn), Tracoef(curmod))
    temod2 <- arimax(y, order = order, xtransf = xtransf,init = cur_init,
...
OptimInarimax <-function(y,order,xtransf,transfer)
{
  ...
  armalist <- order
  inittra <-c(0)
  mod <- try(arimax(y,order = order, xtransf = xtransf, transfer= transfer, method = "ML"))
  if (inherits(mod, "try-error")){
    ...
    while(Loop & curList[1]<5 & curList[3]<5)
    {
      ...
      newmod <- try(arimax(y,order = curList,xtransf = xtransf, transfer= transfer, method = "ML"))
...
auto.ivarima <- function(y,its_start,LTFmaxk)
{
  ...
  if(!is.null(TFptMod)){
    while(resloop & loopi<6)
    {
      ...
      {
        autores <- auto.arima(TFptMod$residuals)
    ...
    Xppeak <-find_max_v(TFptMod,LTFmaxk)
    ...
    while (length(UnEvaledlist) > 0)
    {
      ...
      {
        lag <- first$Xdatab
        first$Xdata <- lag(zoo(first$Xdata), -lag, na.pad = TRUE)
        first$Xdata <- ifelse(is.na(first$Xdata), 0, first$Xdata)
      }
      bestmod <-try(OptimInarimax(y, order = c(1, 0, 0),xtransf = first$Xdata,
                                  transfer = list(c(first$Xdatar,first$Xdatah))))
      if (inherits(bestmod, "try-error")) {
        UnEvaledlist[[1]] <-NULL
      } else {
        coefz <- coeftest(bestmod)
        ...
        Evaledlist[[first_keys]] <- list(aic = bestmod$aic,
